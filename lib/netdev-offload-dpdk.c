                  ...
                  const struct rte_flow_item *item)
{
    ...
    if (item->type == RTE_FLOW_ITEM_TYPE_ETH) {
        ...
        if (vlan_spec) {
            ...
            ds_put_format(s1,
                          "inner_type spec 0x%"PRIx16" tci spec 0x%"PRIx16" ",
                          ntohs(vlan_spec->inner_type), ntohs(vlan_spec->tci));
        } else {
            ds_put_cstr(s, "  Spec = null\n");
        }
        ...
        if (vlan_mask) {
            ...
            ds_put_format(s1,
                          "inner_type mask 0x%"PRIx16" tci mask 0x%"PRIx16" ",
                          ntohs(vlan_mask->inner_type), ntohs(vlan_mask->tci));
        } else {
            ds_put_cstr(s, "  Mask = null\n");
        }
        ...
        if (icmp_spec) {
            ...
            ds_put_format(s1,
                          "type spec %"PRIu8" code spec %"PRIu8" ",
                          icmp_spec->hdr.icmp_type,
                          icmp_spec->hdr.icmp_code);
        } else {
            ds_put_cstr(s, "  Spec = null\n");
        ...
        if (icmp_mask) {
            ...
            ds_put_format(s1,
                          "type spec %"PRIu8" code mask %"PRIu8" ",
                          icmp_mask->hdr.icmp_type,
                          icmp_mask->hdr.icmp_code);
        } else {
            ds_put_cstr(s, "  Mask = null\n");
        ...
        ds_put_cstr(s, "rte flow ipv6_ext pattern:\n");
        if (ipv6_frag_spec) {
            ds_put_format(s, "  Spec: next_header=%"PRIu8", "
                          "frag_data=0x%"PRIx16"\n",
        ...
        if (ipv6_frag_mask) {
            ds_put_format(s, "  Mask: next_header=%"PRIu8", "
                          "frag_data=0x%"PRIx16"\n",
        ...
        if (ipv6_frag_last) {
            ds_put_format(s, "  Last: next_header=%"PRIu8", "
                          "frag_data=0x%"PRIx16"\n",
    ...
    if (actions->type == RTE_FLOW_ACTION_TYPE_MARK) {
        ...
        if (count) {
            ds_put_format(s, "  Count: shared=%d, id=%d\n", count->shared,
                          count->id);
            ds_put_format(s1, "shared %d identifier %d ", count->shared,
                          count->id);
        } else {
